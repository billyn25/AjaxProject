    ...
    return (
        <div id="mySidenav" className="sidenav">
            ...
            <ul className="productsBar m-0">
                <div>
                    <input type="checkbox" className="flipswitch bandwbut" id="customCheck1"
            ...
            <ul className="productsBar m-0 pt-0 pb-0">
                {Object.keys(arrayFiltered).map((key, index) => {
                    return arrayFiltered[key].cat ==='kit' ?
                        <li className="menuImg" tabIndex="0" data-bs-toggle="tooltip" title={arrayFiltered[key].description}
                        key={key} onClick={() => checkIfExist(arrayFiltered[key].title)}>
                        <img alt={arrayFiltered[key].filename} className="m-2" src={arrayFiltered[key].filename}
                        width='40' height='40'/>
